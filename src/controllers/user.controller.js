...
const getUserChannelProfile = asyncHandler(async (req , res) => {
    //subscription ko join krna ha users ka andar
    ...
    const channel = await User.aggregate([
        ...
        },//at match we have only one user now we have t lookup
        {
            $lookup:{
                from: "subscriptions",
                localField: "_id",
    ...
    if(!channel?.length){
        throw new ApiError(404,"channe doesnot exist")
    }
    return res
    .status(200)
    .json(
...
//watched history array of ids videos but in videos owner we have to looku nested lookup
    ...
    const user = await Users.aggregate([
        {
            ...
            $lookup: {
                ...
                foreignField: "_id",
                as : "watchHistory",//sub pipline warna owners get ni ho ga videos ma sa
                pipeline: [
                    {
                        $lookup: {
                            from:"users",
                    ...
                    {
                        //array and first val is important
                        $addFields:{
                            owner:{
                                $first: "$owner"
                            }
